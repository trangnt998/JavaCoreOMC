    ...
    */
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < customer.length; i++) {//tạo danh sách khách hàng để test
            ...
            customer[i][2] = "096000000" + (i + 1);//phone
            electricCustomer[i][0] = (i % 5);//customerType, 0,1,2,3,4 lần lượt là khách lẻ, sỉ, doanh nghiệp cao, trung và thấp áp
            electricCustomer[i][1] = Math.floor(random.nextDouble() * 4000);//electricNumberPrevPeriod
            electricCustomer[i][2] = electricCustomer[i][1] + Math.floor(random.nextDouble() * 1000);//electricNumberThisPeriod
            double tmp = electricCustomer[i][2] - electricCustomer[i][1];
            electricBusiness[i][0] = Math.floor(tmp / 5);//idleElectricNumber
            electricBusiness[i][1] = Math.floor(tmp / 4);//nomalElectricNumber
            electricBusiness[i][2] = tmp - electricBusiness[i][0] - electricBusiness[i][1];//rushElectricNumber
        }
        ...
        for (int c = 0; c < n; c++) {
            if (c % 5 == 0) {
                ...
                WholesaleCustomer wholesaleCustomer = new WholesaleCustomer(customer[c][0], customer[c][1], customer[c][2],
                        electricCustomer[c][0], electricCustomer[c][1], electricCustomer[c][2]);
